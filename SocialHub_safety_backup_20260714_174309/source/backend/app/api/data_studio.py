...
async def get_users(
    ...
    for u in users:
        user_list.append({
            ...
            "created_at": isoformat_utc_z(u.created_at)
        })
...
async def seed_10k_data(
    ...
    batch = DemoDataBatch(
        id=batch_id,
        batch_name=f"Demo Batch {utcnow_naive().strftime('%Y-%m-%d %H:%M')}",
...
async def get_original_media(
    ...
    for a in assets:
        asset_list.append({
            ...
            "created_at": isoformat_utc_z(a.created_at)
        })
...
async def get_analytics(
    ...
    for log in recent_logs:
        logs.append({
            ...
            "source": log.source,
            "status": log.status,
            "created_at": isoformat_utc_z(log.created_at)
        })
