...
async function loadFeed(page = 1) {
    ...
    if (!feed || !isLoggedIn()) { if (feed) feed.innerHTML = '<div class="empty-state"><h2>Welcome to SocialHub</h2><p>Login to see your feed</p><a href="/login" class="btn btn-primary">Login</a></div>'; return; }
    try {
        feed.innerHTML = renderPostSkeletons();
        const data = await api(`/posts?page=${page}`);
        if (!data.posts || data.posts.length === 0) { feed.innerHTML = '<div class="empty-state"><i class="fas fa-newspaper fa-3x"></i><h3>No posts yet</h3><p>Follow people to see their posts here!</p></div>'; return; }
        feed.innerHTML = data.posts.map(p => renderPost(p)).join('');
        if (data.has_next) feed.innerHTML += `<button class="btn btn-outline center" onclick="loadFeed(${page + 1})">Load More</button>`;
    } catch (e) { feed.innerHTML = '<div class="empty-state">Error loading feed</div>'; }
}
...
function renderPost(post) {
    ...
    const images = (post.images || []).map(img =>
        img.is_video ? `<video src="${mediaUrl(img.video_url || img.image_url)}" controls class="post-media"></video>` :
        `<img src="${mediaUrl(img.image_url)}" class="post-media" loading="lazy">`
    ).join('');
    ...
    return `<div class="post-card" id="post-${post.id}">
        <div class="post-header">
            ...
            <div class="content-menu"><button class="post-menu-btn" onclick="toggleContentMenu(this)"><i class="fas fa-ellipsis-h"></i></button>
                <div class="content-menu-dropdown hidden">${canDelete ? `<button onclick="deleteContent('post','${post.id}')"><i class="fas fa-trash"></i> Delete</button>` : '<button disabled>Report</button>'}</div>
            </div>
        </div>
...
function initTheme() {
    ...
    document.querySelectorAll('.theme-toggle').forEach(btn => {
        btn.setAttribute('aria-label', `Switch to ${saved === 'light' ? 'dark' : 'light'} mode`);
        btn.setAttribute('title', `Switch to ${saved === 'light' ? 'dark' : 'light'} mode`);
    });
    document.querySelectorAll('.theme-toggle i').forEach(i => i.className = saved === 'light' ? 'fas fa-moon' : 'fas fa-sun');
}
...
async function toggleLike(postId) {
    try { await api(`/likes/${postId}`, { method: 'POST', body: JSON.stringify({ reaction: 'like' }) }); loadFeed(); } catch (e) { try { await api(`/likes/${postId}`, { method: 'DELETE' }); loadFeed(); } catch (e2) {} }
}
...
async function sharePost(postId) {
    try { await api(`/posts/${postId}/repost`, { method: 'POST', body: JSON.stringify({}) }); showToast('Post shared!', 'success'); loadFeed(); } catch (e) { showToast(e.message || 'Error sharing post', 'error'); }
}
...
function renderReel(reel) {
    ...
    if (!src) return '<div class="reel-card"><div class="empty-state"><p>Video unavailable</p></div></div>';
    ...
    return `<div class="reel-card" id="reel-${reel.id}">
        ...
        <div class="reel-video-error"><i class="fas fa-triangle-exclamation"></i> Video could not load. Check that ${escapeHtml(src)} exists.</div>
...
function toggleReelSound(button) {
    ...
    if (video && reelsSoundEnabled) video.play().catch(() => {});
}
...
async function loadNotifications() {
    ...
    try {
        ...
        if (notifs.length === 0) { container.innerHTML = '<div class="empty-state"><i class="fas fa-bell-slash fa-3x"></i><h3>No notifications yet</h3><p>When someone interacts with you, you\'ll see it here</p></div>'; return; }
        container.innerHTML = notifs.map(n => {
            ...
            else if (notifType === 'follow') { iconClass = 'follow'; icon = 'fa-user-plus'; }
            else if (notifType === 'follow_request') { iconClass = 'follow'; icon = 'fa-user-check'; }
            ...
            return `<div class="notif-item ${n.is_read ? '' : 'unread'}" onclick="markNotifRead('${n.id}')">
                ...
                <img src="${getProfilePic(actor)}" class="avatar-sm">
    ...
    } catch (e) { container.innerHTML = '<div class="empty-state">Error loading notifications</div>'; }
}
...
function initPageAnimations() {
    ...
    // Click ripple effect for buttons/nav actions
    document.addEventListener('click', (event) => {
        const button = event.target.closest('.btn, .action-btn, .nav-icon, .admin-sidebar a, .settings-sidebar a, .menu-list a');
        if (!button) return;
...
async function loadInstagramStatus(){ const el=document.getElementById('instagram-status'); try{ const d=await api('/instagram/account'); if(el) el.innerHTML=`<div class="mini-row"><span>Status</span><strong>${d.connected?'Connected':'Not connected'}</strong></div>`; }catch(e){ if(el) el.innerHTML='<div class="mini-row"><span>Status</span><strong>Login required</strong></div>'; } }
async function loadInstagramStudioPage(){ await loadInstagramStatus(); const grid=document.getElementById('instagram-media-grid'); if(!grid) return; try{ const d=await api('/instagram/media'); grid.innerHTML=(d.media||[]).map(m=>`<div class="media-card"><img src="${m.thumbnail_url||m.media_url||'/static/images/default_cover.svg'}"><strong>${escapeHtml(m.media_type)}</strong><div class="btn-group"><button class="btn btn-sm btn-outline" onclick="api('/instagram/import/${m.id}/post',{method:'POST'}).then(()=>showToast('Imported as post'))">Post</button><button class="btn btn-sm btn-outline" onclick="api('/instagram/import/${m.id}/reel',{method:'POST'}).then(()=>showToast('Imported as reel'))">Reel</button></div></div>`).join('') || '<div class="empty-state">No Instagram media imported yet.</div>'; }catch(e){ grid.innerHTML='<div class="empty-state">Connect Instagram to import media.</div>'; } }
...
// ==================== SocialHub UI/UX Enhancement Initializer ====================
// Guarded, additive enhancements only: preserves existing API calls, element IDs,
// form actions, upload handlers, routes, and websocket logic.
(function initSocialHubFrontendEnhancements() {
    if (window.__socialHubFrontendEnhancementsLoaded) return;
    window.__socialHubFrontendEnhancementsLoaded = true;
    ...
    function initResponsiveNavigation() {
        if (!document.querySelector('.navbar') || document.getElementById('mobile-bottom-nav')) return;
        const path = window.location.pathname;
        const active = path === '/' ? 'home' : path.includes('explore') || path.includes('search') ? 'explore' : path.includes('reels') ? 'reels' : path.includes('chat') ? 'chat' : path.includes('profile') ? 'profile' : '';
        const profileHref = currentUser?.username ? `/profile/${currentUser.username}` : '/settings';
        ...
        nav.innerHTML = [
            ['/', 'fa-home', 'Home', 'home'],
            ['/explore', 'fa-compass', 'Explore', 'explore'],
            ['/reels', 'fa-video', 'Reels', 'reels'],
            ['#create', 'fa-plus-square', 'Create', 'create'],
            ['/chat', 'fa-paper-plane', 'Chat', 'chat'],
            [profileHref, 'fa-user', 'Profile', 'profile']
        ].map(([href, icon, label, key]) => href === '#create'
            ? `<button type="button" aria-label="Create post" data-mobile-create><i class="fas ${icon}"></i><span>${label}</span></button>`
            : `<a href="${href}" class="${active === key ? 'active' : ''}" aria-label="${label}"><i class="fas ${icon}"></i><span>${label}</span></a>`
        ).join('');
        document.body.appendChild(nav);
        nav.querySelector('[data-mobile-create]')?.addEventListener('click', (e) => { e.preventDefault(); if (typeof showCreatePost === 'function') showCreatePost(); });
    }

    function initTheme() {
        const saved = localStorage.getItem('theme') || document.documentElement.dataset.theme || 'dark';
        document.documentElement.dataset.theme = saved;
        document.querySelectorAll('.theme-toggle').forEach(btn => {
            if (!once(btn, 'themeBound')) return;
            ...
            btn.addEventListener('click', () => {
                ...
                setTimeout(() => btn.classList.remove('animate-scale-in'), 320);
            });
        });
    }

    function initScrollAnimations(root = document) {
        const selectors = '.post-card,.stories-card,.create-post-box,.profile-header-card,.profile-post-grid-item,.profile-reel-grid-item,.product-card,.collab-card,.chart-card,.stat-card,.notification-item,.user-card,.hashtag-card,.reel-card-sm,.media-card,.collection-card,.live-card,.music-page-card';
        const items = [...root.querySelectorAll(selectors)].filter(el => !el.classList.contains('is-visible'));
        items.forEach((item, index) => {
            item.classList.add('stagger-item');
            item.style.setProperty('--stagger-delay', `${Math.min(index % 12, 10) * 45}ms`);
        });
        if (prefersReducedMotion() || !('IntersectionObserver' in window)) {
            items.forEach(el => el.classList.add('is-visible'));
            return;
        }
        if (!window.__socialHubRevealObserver) {
            window.__socialHubRevealObserver = new IntersectionObserver(entries => {
                entries.forEach(entry => {
                    if (!entry.isIntersecting) return;
                    entry.target.classList.add('is-visible');
                    window.__socialHubRevealObserver.unobserve(entry.target);
                });
            }, { threshold: 0.12, rootMargin: '0px 0px -40px 0px' });
        }
        items.forEach(el => window.__socialHubRevealObserver.observe(el));
    }

    function initGridAnimations(root = document) {
        root.querySelectorAll('.product-grid,.collab-grid,.dashboard-grid,.profile-posts-grid,.profile-reels-grid,.posts-grid,.reels-grid,.media-grid,.pro-grid').forEach(grid => {
            [...grid.children].forEach((child, index) => child.style.setProperty('--stagger-delay', `${Math.min(index % 12, 10) * 45}ms`));
        });
    }

    function initMediaLazyLoading(root = document) {
        root.querySelectorAll('img').forEach((img, index) => {
            if (!img.hasAttribute('decoding')) img.setAttribute('decoding', 'async');
            if (!img.hasAttribute('loading') && index > 2 && !img.closest('.profile-avatar-wrap')) img.setAttribute('loading', 'lazy');
            if (!img.complete) img.classList.add('img-loading');
            if (once(img, 'imageLoadBound')) {
                img.addEventListener('load', () => { img.classList.remove('img-loading'); img.classList.add('img-loaded'); }, { once: true });
                img.addEventListener('error', () => { img.src = '/static/images/default_avatar.svg'; img.classList.add('img-loaded'); }, { once: true });
            }
        });
    ...
    function initImageFallbacks(root = document) { initMediaLazyLoading(root); }

    function initModalAnimations() {
        document.querySelectorAll('.modal-overlay').forEach(modal => {
            if (!once(modal, 'modalBound')) return;
            modal.addEventListener('transitionend', () => document.body.classList.toggle('modal-open', !!document.querySelector('.modal-overlay.active,.story-viewer.active,.media-viewer.active')));
        });
        document.addEventListener('keydown', (e) => {
            if (e.key !== 'Escape') return;
            document.querySelectorAll('.modal-overlay.active,.story-viewer.active,.media-viewer.active').forEach(el => el.classList.remove('active'));
            document.body.classList.remove('modal-open');
        });
        const oldOpenModal = window.openModal;
        window.openModal = function(id) {
            oldOpenModal ? oldOpenModal(id) : document.getElementById(id)?.classList.add('active');
            document.body.classList.add('modal-open');
        };
        const oldCloseModal = window.closeModal;
        window.closeModal = function(id) {
            oldCloseModal ? oldCloseModal(id) : document.getElementById(id)?.classList.remove('active');
            if (!document.querySelector('.modal-overlay.active,.story-viewer.active,.media-viewer.active')) document.body.classList.remove('modal-open');
        };
    }

    function initReelsObserver(root = document) {
        const reels = [...root.querySelectorAll('.reel-card')];
        if (!reels.length || !('IntersectionObserver' in window)) return;
        reels.forEach(card => {
            if (!card.querySelector('.reel-progress')) card.insertAdjacentHTML('afterbegin', '<div class="reel-progress"><span></span></div>');
            const video = card.querySelector('video');
            if (video && once(video, 'reelProgressBound')) {
                video.addEventListener('timeupdate', () => {
                    const bar = card.querySelector('.reel-progress span');
                    if (bar && video.duration) bar.style.width = `${(video.currentTime / video.duration) * 100}%`;
                });
            }
        });
        if (window.__socialHubReelsObserver) window.__socialHubReelsObserver.disconnect();
        window.__socialHubReelsObserver = new IntersectionObserver(entries => {
            entries.forEach(entry => {
                const video = entry.target.querySelector('video');
                if (!video) return;
                if (entry.isIntersecting && entry.intersectionRatio > 0.62) {
                    root.querySelectorAll('.reel-card video').forEach(v => { if (v !== video) v.pause(); });
                    video.play().catch(() => {});
                } else {
                    video.pause();
                }
        ...
        reels.forEach(card => window.__socialHubReelsObserver.observe(card));
    }

    function initStoryViewer() {
        document.querySelectorAll('.story-viewer').forEach(viewer => {
            if (!once(viewer, 'storyViewerBound')) return;
    ...
    function initTabAnimations() {
        document.addEventListener('click', e => {
            const tab = e.target.closest('.profile-tab-btn,.explore-tab,.search-tab,.tab');
            if (!tab) return;
            tab.classList.add('animate-scale-in');
            setTimeout(() => tab.classList.remove('animate-scale-in'), 260);
        });
    }

    function initToastNotifications() {
        const oldShowToast = window.showToast;
        if (typeof oldShowToast !== 'function' || oldShowToast.__enhanced) return;
        const enhanced = function(message, type = 'info') {
            oldShowToast(message, type);
            setTimeout(() => document.querySelectorAll('.toast').forEach(t => t.style.animation = 'toastIn 260ms ease both'), 0);
        };
        enhanced.__enhanced = true;
        window.showToast = enhanced;
    }
    ...
    window.initGridAnimations = initGridAnimations;
    window.initModalAnimations = initModalAnimations;
    window.initMediaLazyLoading = initMediaLazyLoading;
    window.initImageFallbacks = initImageFallbacks;
    window.initTheme = initTheme;
    window.initReelsObserver = initReelsObserver;
    window.initStoryViewer = initStoryViewer;
    window.initTabAnimations = initTabAnimations;
    window.initToastNotifications = initToastNotifications;

    const oldLoadFeed = window.loadFeed;
    if (typeof oldLoadFeed === 'function' && !oldLoadFeed.__enhanced) {
        const wrapped = async function(...args) { const result = await oldLoadFeed.apply(this, args); enhanceDynamicContent(); return result; };
        wrapped.__enhanced = true;
        window.loadFeed = wrapped;
    }

    const oldLoadReels = window.loadReels;
    if (typeof oldLoadReels === 'function' && !oldLoadReels.__enhanced) {
        const wrapped = async function(...args) { const result = await oldLoadReels.apply(this, args); enhanceDynamicContent(); return result; };
        wrapped.__enhanced = true;
        window.loadReels = wrapped;
    }
    ...
    function boot() {
        ...
        setTimeout(enhanceDynamicContent, 450);
    }
