...
class PostShareRequest(BaseModel):
    channel: str = "socialhub"


class PostPreferenceRequest(BaseModel):
    preference: str
...
def share_post_lightweight(
    post_id: str,
    request: PostShareRequest = PostShareRequest(),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Track a post share without creating a repost, preserving the older /repost behavior."""
    post = db.query(Post).filter(Post.id == post_id, Post.is_deleted == False).first()
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")
    ...
    db.commit()
    return {"success": True, "shares_count": db.query(PostShare).filter(PostShare.post_id == post_id).count()}


@router.post("/{post_id}/hide")
def hide_post_for_user(post_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """Session-compatible hide endpoint. It validates the post and lets the UI hide it immediately."""
    post = db.query(Post).filter(Post.id == post_id, Post.is_deleted == False).first()
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")
...
@router.post("/{post_id}/preference")
def set_post_preference(
    post_id: str,
    request: PostPreferenceRequest,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Accept Instagram-style Interested / Not Interested signals for ranking compatibility."""
    post = db.query(Post).filter(Post.id == post_id, Post.is_deleted == False).first()
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")
    if request.preference not in {"interested", "not_interested"}:
        raise HTTPException(status_code=400, detail="Invalid preference")
    return {"success": True, "post_id": post_id, "preference": request.preference}


@router.post("/{post_id}/report")
def report_post_compat(
    post_id: str,
    request: PostReportRequest,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Create a lightweight report through the post menu without breaking the existing reports API."""
    ...
    report = Report(
        ...
        reason=(request.reason or "spam").lower().replace(" ", "_")[:50],
        description=request.details or f"Reported from post menu by @{current_user.username}"
    )
    db.add(report)
    db.commit()
...
def get_post_insights(post_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """Return safe post insights for owners/admins and basic public metrics for viewers."""
    post = db.query(Post).filter(Post.id == post_id, Post.is_deleted == False).first()
    if not post:
        raise HTTPException(status_code=404, detail="Post not found")
    shares = db.query(PostShare).filter(PostShare.post_id == post_id).count()
    return {
        "post_id": post_id,
        "likes_count": post.likes_count,
        "comments_count": post.comments_count,
        "shares_count": shares,
        "saves_count": db.query(Bookmark).filter(Bookmark.post_id == post_id).count(),
        "reach_estimate": max(post.likes_count * 8 + post.comments_count * 12 + shares * 15, 1),
        "engagement_rate": round((post.likes_count + post.comments_count + shares) / max(1, post.likes_count * 8 + 1) * 100, 2),
    }
