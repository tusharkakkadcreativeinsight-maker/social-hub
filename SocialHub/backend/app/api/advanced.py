...
def _fallback_bio(name: str = "", niche: str = "", vibe: str = "professional"):
    samples = [
        f"{name} | {niche.title() if niche else 'Creator'}\nâœ¨ Sharing my journey\nðŸ“© Collabs: DM me\nâ¬‡ï¸ Check out my latest",
        f"ðŸ‘‹ Hey, I'm {name}!\n{'ðŸŽ¯ ' + niche.title() if niche else ''}\nðŸš€ Building in public\nðŸ’¬ Let's connect!",
        f"{name} â€¢ {niche.title() if niche else 'Creator'}\nðŸŒŸ Living my best life\nðŸ“¸ Capturing moments\nðŸ‘‡ Follow for more",
    ]
...
def _fallback_reel_title(topic: str = "", keywords: str = ""):
    titles = [
        f"POV: {topic or 'You finally'} ðŸŽ¬",
        f"{topic or 'This'} is INSANE ðŸ”¥",
        f"The Ultimate {topic or 'Vibe'} ðŸš€",
        f"{topic or 'New'} Era Begins âœ¨",
        f"You Won't Believe This {topic or 'Moment'} ðŸ˜±",
    ]
...
def _fallback_hooks(topic: str = "", audience: str = "creators"):
    ...
    hooks = [
        ...
        f"I tried {base} for 7 days â€” here is what changed",
...
def _fallback_comment_reply(comment: str, tone: str = "friendly", context: str = ""):
    tone_prefix = {
        "professional": "Thanks for sharing your perspective â€”",
        "funny": "Haha, I love this ðŸ˜‚",
    ...
    if context:
        reply += f" Iâ€™ll share more about {context[:80]} soon."
    return {"reply": reply, "alternatives": [reply, "Great point â€” thanks for being here!", "Love this feedback, thank you ðŸ™Œ"], "source": "local_fallback"}


def _fallback_image_prompt(prompt: str, style: str, image_info: dict):
    clean_prompt = (prompt or "enhance this social media image").strip()
    clean_style = (style or "natural").strip().lower()
    revised = (
        f"Edit the uploaded image while preserving the main subject and identity. "
        f"Apply this request: {clean_prompt}. Use a {clean_style} creator-ready style, "
        "balanced lighting, clean composition, realistic details, high quality, and social-media friendly framing. "
        "Avoid distortion, extra fingers, warped faces, unreadable text, or changing protected identity traits."
    )
    steps = [
        "Read the uploaded image and keep the original subject/composition recognizable.",
        f"Apply the requested change: {clean_prompt}.",
        f"Match the selected style: {clean_style}.",
        "Improve lighting, contrast, sharpness, and background polish for a post/reel cover.",
        "Export as a clean JPG/PNG ready for SocialHub upload.",
    ]
    return {
        ...
        "edit_steps": steps,
        "safety_notes": [
            "Only edit images you own or have permission to use.",
            "Do not request harmful, explicit, deceptive, or impersonation edits.",
        ],
...
@router.post("/api/ai/image-prompt", response_model=ImagePromptResponse)
async def generate_image_prompt(
    ...
    """Read an uploaded image and return a stronger prompt for AI image editing.

    This endpoint validates the image, extracts safe metadata with Pillow, and
    returns edit instructions. If OpenAI is configured, it asks for a refined
    image-edit prompt; otherwise it uses a deterministic local fallback so the
    feature works in local development.
    """
    if not validate_image_file(image):
        raise HTTPException(status_code=400, detail="Upload a JPG, PNG, or WEBP image under the configured image size limit")
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Could not read image content")

    fallback = _fallback_image_prompt(prompt, style, image_info)
    api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        return fallback
    ...
    try:
        ...
        completion = client.chat.completions.create(
            ...
            messages=[{
                "role": "user",
                "content": (
                    "Rewrite this user request into one safe, detailed image-editing prompt. "
                    "Preserve the original subject and identity, mention composition/lighting/style, "
                    "and avoid unsafe or deceptive edits. "
                    f"Image metadata: {image_info}. Style: {style}. User request: {prompt}"
                ),
            }],
            temperature=0.7,
        )
        ...
        if ai_text:
            fallback["revised_prompt"] = ai_text
            fallback["source"] = "openai"
        return fallback
    except Exception:
        return fallback
