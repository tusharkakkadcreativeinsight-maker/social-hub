...
from ..utils.time import isoformat_utc_z, utc_now, utcnow_naive
...
def make_state(user_id: str) -> str:
    return jwt.encode({"sub": user_id, "purpose": "instagram_oauth", "exp": utc_now() + timedelta(minutes=15)}, settings.SECRET_KEY, algorithm=settings.ALGORITHM)
...
def account_dict(a: InstagramAccount):
    return {"id": a.id, "instagram_user_id": a.instagram_user_id, "username": a.username, "profile_picture_url": a.profile_picture_url, "account_type": a.account_type, "token_expires_at": isoformat_utc_z(a.token_expires_at), "connected_at": isoformat_utc_z(a.connected_at)}


def media_dict(m: InstagramMedia):
    return {"id": m.id, "instagram_media_id": m.instagram_media_id, "media_type": m.media_type, "media_url": m.media_url, "thumbnail_url": m.thumbnail_url, "caption": m.caption, "permalink": m.permalink, "timestamp": isoformat_utc_z(m.timestamp), "like_count": m.like_count, "comments_count": m.comments_count, "imported_at": isoformat_utc_z(m.imported_at)}
...
def upsert_media(db: Session, account: InstagramAccount, raw: Dict[str, Any], imported=False):
    ...
    if imported:
        media.imported_at = utcnow_naive()
    db.add(media)
    return media
...
def instagram_callback(code: str = Query(None), state: str = Query(None), db: Session = Depends(get_db)):
    ...
    account.token_expires_at = utcnow_naive() + timedelta(seconds=int(token_data.get("expires_in") or 5184000))
    account.connected_at = utcnow_naive()
    db.add(account)
    db.commit()
...
def import_media(media_ids: List[str], db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    for mid in media_ids:
        ...
        if m:
            m.imported_at = utcnow_naive()
            imported.append(m)
    db.add(InstagramImportLog(user_id=current_user.id, instagram_account_id=account.id, action="import_selected", requested_count=len(media_ids), imported_count=len(imported), status="success", completed_at=utcnow_naive()))
    db.commit()
...
def import_one(media_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if m.media_type == "VIDEO" and m.permalink and "/reel/" in m.permalink:
        db.add(InstagramReel(user_id=current_user.id, instagram_account_id=account.id, instagram_media_id=m.instagram_media_id, media_url=m.media_url, thumbnail_url=m.thumbnail_url, caption=m.caption, permalink=m.permalink, timestamp=m.timestamp, imported_at=utcnow_naive()))
    db.add(InstagramImportLog(user_id=current_user.id, instagram_account_id=account.id, action="import_one", requested_count=1, imported_count=1, status="success", completed_at=utcnow_naive()))
    db.commit()
