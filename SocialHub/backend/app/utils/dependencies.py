...
def validate_video_file(file: UploadFile) -> bool:
    ...
    allowed_types = [
        ...
        # Some browsers/devices send valid video files with generic MIME types.
...
def validate_audio_file(file: UploadFile) -> bool:
    """Validate uploaded audio/music file."""
    allowed_types = ["audio/mpeg", "audio/wav", "audio/x-wav", "audio/ogg", "audio/mp4", "audio/x-m4a", "application/octet-stream"]
    if file.content_type and file.content_type not in allowed_types:
        return False
    ...
    try:
        if hasattr(file, 'size') and file.size is not None and file.size > settings.MAX_UPLOAD_SIZE:
            return False
    except Exception:
        pass
...
async def save_upload_file(upload_dir: str, file: UploadFile, subdir: str = "") -> str:
    ...
    subfolders (profiles, covers, posts, reels, stories, chat,
    ...
    """
    # The public contract is /uploads/chat/<file>. Older code may still pass
    # chat_files; normalize it at save-time so the database stores only chat/...
    clean_subdir = "chat" if subdir == "chat_files" else subdir
    base_upload_dir = os.path.abspath(settings.UPLOAD_DIR)
    save_dir = _validate_upload_destination(upload_dir, clean_subdir)
    os.makedirs(save_dir, exist_ok=True)

    ext = os.path.splitext(file.filename)[1].lower() if file.filename else ".jpg"
    allowed_extensions = set(settings.ALLOWED_EXTENSIONS)
    # Music uploads are validated by validate_audio_file() before saving. Keep
    # these core audio extensions allowed even when a local .env narrows the
    # generic ALLOWED_EXTENSIONS list for image/video uploads.
    if clean_subdir == "music":
        allowed_extensions.update({"mp3", "wav", "m4a", "ogg"})
    if ext.lstrip(".").lower() not in allowed_extensions and clean_subdir != "chat":
    ...
    content = await file.read()
    is_video_upload = (file.content_type or "").startswith("video/") or ext in [".mp4", ".mov", ".webm"]
    is_audio_upload = (file.content_type or "").startswith("audio/") or ext in [".mp3", ".wav", ".m4a", ".ogg"]
    max_size = settings.MAX_VIDEO_SIZE if is_video_upload else settings.MAX_UPLOAD_SIZE
    ...
    try:
        ...
        if clean_subdir:
            return os.path.join(clean_subdir, filename).replace("\\", "/")
