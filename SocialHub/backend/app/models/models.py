...
# Save collection items association table
class CollectionItem(Base):
    __tablename__ = 'collection_items'
    __table_args__ = (
        UniqueConstraint('collection_id', 'post_id', name='unique_collection_post'),
        UniqueConstraint('collection_id', 'reel_id', name='unique_collection_reel'),
    )

    id = Column(String, primary_key=True, default=generate_uuid)
    collection_id = Column(String, ForeignKey('saved_collections.id', ondelete='CASCADE'), nullable=False, index=True)
...
class User(Base):
    ...
    music_tracks = relationship("Music", back_populates="user", cascade="all, delete-orphan", foreign_keys="Music.user_id")
    ...
    verification_request = relationship("VerificationRequest", uselist=False, back_populates="user", cascade="all, delete-orphan", foreign_keys="VerificationRequest.user_id")

    # Saved collections
    saved_collections = relationship("SavedCollection", back_populates="user", cascade="all, delete-orphan")

    # Profile visits
    profile_visits_received = relationship("ProfileVisit", foreign_keys="[ProfileVisit.visited_user_id]", back_populates="visited_user", cascade="all, delete-orphan")
    profile_visits_made = relationship("ProfileVisit", foreign_keys="[ProfileVisit.visitor_id]", back_populates="visitor", cascade="all, delete-orphan")

    # Wallet / earnings
    wallet = relationship("CreatorWallet", uselist=False, back_populates="user", cascade="all, delete-orphan")
    earnings = relationship("EarningRecord", back_populates="user", cascade="all, delete-orphan")
    payout_requests = relationship("PayoutRequest", back_populates="user", cascade="all, delete-orphan", foreign_keys="PayoutRequest.user_id")

    # Recent searches
    recent_searches = relationship("RecentSearch", back_populates="user", cascade="all, delete-orphan", foreign_keys="RecentSearch.user_id")

    # Online status for chat
    online_statuses = relationship("UserOnlineStatus", uselist=False, back_populates="user", cascade="all, delete-orphan")

    # Messages deleted for me
    deleted_messages = relationship("DeletedMessage", back_populates="user", cascade="all, delete-orphan")
    live_streams = relationship("LiveStream", back_populates="host", cascade="all, delete-orphan")
...
class Story(Base):
    ...
    highlights = relationship("StoryHighlight", back_populates="story", cascade="all, delete-orphan")
...
class Reel(Base):
    ...
    music_id = Column(String, ForeignKey('music.id', ondelete='SET NULL'), nullable=True, index=True)
    ...
    audio_name = Column(String(150), nullable=True)
    location = Column(String(255), nullable=True)
    created_at = Column(DateTime, default=utcnow)
    updated_at = Column(DateTime, default=utcnow, onupdate=utcnow)

    user = relationship("User", back_populates="reels")
    music = relationship("Music", back_populates="reels")
...
class Music(Base):
    ...
    user = relationship("User", back_populates="music_tracks", foreign_keys=[user_id])
...
class ReelComment(Base):
    ...
    reel_id = Column(String, ForeignKey('reels.id', ondelete='CASCADE'), nullable=False)
    user_id = Column(String, ForeignKey('users.id', ondelete='CASCADE'), nullable=False)
    parent_id = Column(String, ForeignKey('reel_comments.id', ondelete='CASCADE'), nullable=True)
    content = Column(Text, nullable=False)
    ...
    replies = relationship("ReelComment", backref=backref("parent", remote_side="ReelComment.id"), cascade="all, delete-orphan")


class ReelView(Base):
    __tablename__ = 'reel_views'
    __table_args__ = (
        Index('idx_reel_view_reel_user', 'reel_id', 'user_id'),
    )
...
class ReelCommentLike(Base):
    ...
    __table_args__ = (
        UniqueConstraint('user_id', 'comment_id', name='unique_reel_comment_like'),
    )

    id = Column(String, primary_key=True, default=generate_uuid)
    comment_id = Column(String, ForeignKey('reel_comments.id', ondelete='CASCADE'), nullable=False)
    user_id = Column(String, ForeignKey('users.id', ondelete='CASCADE'), nullable=False)
    created_at = Column(DateTime, default=utcnow)
...
class Message(Base):
    ...
    deleted_by_users = relationship("DeletedMessage", back_populates="message", cascade="all, delete-orphan")
...
class DeletedMessage(Base):
    ...
    __table_args__ = (
        UniqueConstraint('user_id', 'message_id', name='unique_deleted_message'),
    )
    ...
    message = relationship("Message", back_populates="deleted_by_users")
...
class UserOnlineStatus(Base):
    ...
    user = relationship("User", back_populates="online_statuses")
...
class LiveStream(Base):
    ...
    __table_args__ = (
        Index('idx_live_status_created', 'status', 'created_at'),
        Index('idx_live_host_created', 'host_id', 'created_at'),
    )
    ...
    description = Column(Text, nullable=True)
    status = Column(String(20), default='active', nullable=False)  # active, ended, deleted
    ...
    messages = relationship("LiveChatMessage", back_populates="live", cascade="all, delete-orphan")
...
class LiveChatMessage(Base):
    ...
    id = Column(String, primary_key=True, default=generate_uuid)
    live_id = Column(String, ForeignKey('live_streams.id', ondelete='CASCADE'), nullable=False, index=True)
    user_id = Column(String, ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True)
    message = Column(Text, nullable=False)
    created_at = Column(DateTime, default=utcnow, nullable=False)

    live = relationship("LiveStream", back_populates="messages")
    user = relationship("User")
...
class LiveViewer(Base):
    ...
    __table_args__ = (
        UniqueConstraint('live_id', 'user_id', name='unique_live_viewer'),
    )
    ...
    live = relationship("LiveStream", back_populates="viewers")
    user = relationship("User")
...
# ---------- VERIFICATION REQUEST (Feature 1) ----------
class VerificationRequest(Base):
    ...
    category = Column(String(50), default='creator')  # 'creator', 'public_figure', 'brand', 'other'
    ...
    user = relationship("User", back_populates="verification_request", foreign_keys=[user_id])
...
class SavedCollection(Base):
    ...
    __table_args__ = (
        UniqueConstraint('user_id', 'name', name='unique_collection_name'),
    )
...
# ---------- PROFILE VISIT (Feature 5) ----------
class ProfileVisit(Base):
    __tablename__ = 'profile_visits'
    __table_args__ = (
        Index('idx_profile_visit_visited', 'visited_user_id', 'created_at'),
        Index('idx_profile_visit_visitor', 'visitor_id', 'created_at'),
    )

    id = Column(String, primary_key=True, default=generate_uuid)
    visited_user_id = Column(String, ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True)
    visitor_id = Column(String, ForeignKey('users.id', ondelete='CASCADE'), nullable=True)
    ip_address = Column(String(45), nullable=True)
    country = Column(String(100), nullable=True)
    created_at = Column(DateTime, default=utcnow)

    visited_user = relationship("User", foreign_keys=[visited_user_id], back_populates="profile_visits_received")
    visitor = relationship("User", foreign_keys=[visitor_id], back_populates="profile_visits_made")


# ---------- CREATOR WALLET (Feature 10) ----------
...
class EarningRecord(Base):
    ...
    __table_args__ = (
        Index('idx_earning_user', 'user_id', 'created_at'),
    )

    id = Column(String, primary_key=True, default=generate_uuid)
    user_id = Column(String, ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True)
    amount = Column(Float, nullable=False)
    source = Column(String(50), nullable=False)  # 'reel', 'post', 'collab', 'bonus', 'tip'
...
class PayoutRequest(Base):
    ...
    __table_args__ = (
        Index('idx_payout_user', 'user_id', 'created_at'),
    )

    id = Column(String, primary_key=True, default=generate_uuid)
    user_id = Column(String, ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True)
    amount = Column(Float, nullable=False)
    payment_method = Column(String(50), default='bank_transfer')  # 'bank_transfer', 'paypal', 'upi'
    account_details = Column(Text, nullable=True)
    status = Column(String(20), default='pending', nullable=False)  # 'pending', 'approved', 'completed', 'rejected'
...
# ---------- RECENT SEARCH (Feature 7) ----------
class RecentSearch(Base):
    __tablename__ = 'recent_searches'
    __table_args__ = (
        Index('idx_recent_search_user', 'user_id', 'created_at'),
    )

    id = Column(String, primary_key=True, default=generate_uuid)
    user_id = Column(String, ForeignKey('users.id', ondelete='CASCADE'), nullable=False, index=True)
    search_type = Column(String(20), nullable=False)  # 'user', 'hashtag', 'text'
    query = Column(String(255), nullable=False)
    target_user_id = Column(String, ForeignKey('users.id', ondelete='CASCADE'), nullable=True)
    created_at = Column(DateTime, default=utcnow)

    user = relationship("User", back_populates="recent_searches", foreign_keys=[user_id])
    target_user = relationship("User", foreign_keys=[target_user_id])


# ---------- HASHTAG TREND SCORE (Feature 8) ----------
class HashtagTrend(Base):
    ...
    __table_args__ = (
        Index('idx_hashtag_trend_score', 'hashtag', 'trend_score'),
    )
